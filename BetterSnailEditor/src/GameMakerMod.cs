...
public partial class GameMakerMod
{
    ...
    public void AddMenuItems()
    {
        ...
        data.CreateToggleOption("\"Custom BGM Playlist\nINFO ---------->\"", "customEditorPlaylist", "global.setting_custom_editor_playlist = argument0", "selectedItem = global.setting_custom_editor_playlist", "global.setting_custom_editor_playlist", tooltipScript: "gml_Script_scr_return_input", tooltipArgument: "\"Allows you to play music from a folder of audio files instead of the regular editor theme.\n\nAdd the songs you want to play to this folder:\n\" + global.betterSE_assets + \"Custom Editor Playlist/\"")
        );
        ...
        if(global.setting_multiframe_loading_wires > 0)
            ...
            return ""OFF (All objects load in one frame.)""
        ", 50), */ data.CreateToggleOption("\"Optimized Saving\"", "better_saving", "global.setting_betterSaving = argument0", "selectedItem = global.setting_betterSaving", "global.setting_betterSaving", "gml_Script_scr_return_input", "\"INCREDIBLY optimized level saving using buffers. When testing, a 600k line lvl file saved in 2 seconds.\n\nAUTOMATIC SAVING UPON EXITING DOES NOT WORK WITH THIS ON.\"")/*, data.CreateToggleOption("\"Optimized Wires (EXPERIMENTAL)\"", "optimized_wires", "global.setting_optimized_wires = argument0", "selectedItem = global.setting_optimized_wires", "global.setting_optimized_wires", "gml_Script_scr_return_input", "\"This feature is mainly designed for incredibly large and complex contraptions and probably won't affect the average wire puzzle.\n\n\nNOTE: In the vanilla game, and gates put in a chain will activate one frame at a time UNLESS you connect them in the order they were placed.\nWith the optimized wire system they ALWAYS go one frame at a time.\"") */);
